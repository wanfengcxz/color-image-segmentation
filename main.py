...
parser.add_argument(
    "-g", "--generations", help="Number of generations to run for", type=int, default=10
)
parser.add_argument(
...
parser.add_argument(
    "-w", "--weights", help="Weights used for sga", type=list, default=[1.0, 1.0, 1.0]
)
parser.add_argument(
    "--mode", help="pls choose from [hsi, rgb]", type=str, default="hsi"
)
parser.add_argument(
    "--r", "--reduce_algo", help="dimension reduce, support []", type=str, default=None
)
...
# cv2.waitKey(0)
image = loadmat("training_images/hsi/PaviaU.mat")["paviaU"]  # (610, 340, 103)
...
if args.algorithm == "nsga":
    ...
    for i, individual in enumerate(population[np.where(front_assignment == 1)]):
        ...
        plt.imsave(
            os.path.join(color_dir, f"pareto_{i}.png"), to_color_segmentation(phenotype)
        )
