...
# 计算XB指数
def calculateXBIndex(membership_mat, cluster_centers, m):
    ...
    for i in range(len(cluster_centers)):
        for j in range(len(cluster_centers)):
            if i != j:
                dist = np.linalg.norm(np.array(cluster_centers[i]) - np.array(cluster_centers[j]))
    ...
    for i in range(num_samples):
        for j in range(len(cluster_centers)):
            xb_index += (membership_mat[i][j] ** m) * (np.linalg.norm(np.array(dataset.iloc[i]) - np.array(cluster_centers[j])) ** 2)
...
if __name__ == '__main__':
    ...
    # F_measure, ACC, NMI, RI, ARI = calculateXBIndex(membership, centers, m)  # 计算聚类指标
    res = calculateXBIndex(membership, centers, m)
    print(res)
    ...
    # cluster_labels, cluster_centers, membership_mat = fuzzyCMeansClustering()
    # draw_cluster(dataset, cluster_centers, cluster_labels)